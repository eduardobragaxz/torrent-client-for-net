...
public class AnnounceResponseMessage : TrackerMessage
{
    ...
    public override int Length
    {
        get
        {
            return ActionLength + TransactionIdLength + IntervalLength + LeechersLength + SeedersLength + (Peers.Count() * (IpAddressLength + PortLength));
        }
    }
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        Message.Write(buffer, ref written, (int)Interval.TotalSeconds);
        Message.Write(buffer, ref written, LeecherCount);
        Message.Write(buffer, ref written, SeederCount);

        foreach (IPEndPoint peerEndpoint in Peers)
        {
