...
public class AnnounceResponseMessage
{
    ...
    public static bool TryDecode(byte[] data, out AnnounceResponseMessage message)
    {
        ...
        IPEndPoint endpoint;
        BEncodedString failureReasonKey = new("failure reason");
        BEncodedString intervalKey = new("interval");
        BEncodedString completeKey = new("complete");
        BEncodedString incompleteKey = new("incomplete");
        BEncodedString peersKey = new("peers");
        BEncodedString peerIdKey = new("peer id");
        BEncodedString ipaddressKey = new("ip");
        BEncodedString portKey = new("port");
                    ...
                    foreach (var item in value.As<BEncodedDictionary>()[peersKey].As<BEncodedList>())
                    {
                        if (item is BEncodedDictionary)
                        {
                                ...
                                item.As<BEncodedDictionary>()[portKey] is BEncodedNumber)
                            {
                                ...
                                if (IPAddress.TryParse(peerIp, out IPAddress tmpIpAddress) &&
