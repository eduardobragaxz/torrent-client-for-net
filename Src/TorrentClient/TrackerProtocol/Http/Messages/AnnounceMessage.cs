...
public class AnnounceMessage
{
    ...
    public string Encode()
    {
        ...
        {
            { "info_hash", HttpUtility.UrlEncode(InfoHash.ToByteArray()) },
            { "peer_id", Encoding.ASCII.GetString(Message.FromPeerId(PeerId)) },
            { "port", Port },
            { "uploaded", BytesUploaded },
            { "downloaded", BytesDownloaded },
            { "left", BytesLeft },
            { "numwant", PeersWantedCount },
            { "event", TrackingEvent.ToString().ToLower(CultureInfo.InvariantCulture) }
        };
