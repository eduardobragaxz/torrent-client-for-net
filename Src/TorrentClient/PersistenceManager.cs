...
public sealed class PersistenceManager : IDisposable
{
    ...
    /// </summary>
    private readonly long torrentLength;
    ...
    {
        PieceStatus[] bitField = new PieceStatus[pieceHashes!.Count()];
        long pieceStart;
        long pieceEnd;
        ...
        lock (locker)
        {
            foreach (var file in files!)
            {
                ...
                for (long pieceIndex = pieceStart; pieceIndex <= pieceEnd; pieceIndex++)
                {
                    ...
                    {
                        bitField[previousPieceIndex] = GetStatus(ignore, download, pieceHashes!.ElementAt((int)previousPieceIndex), pieceData.CalculateSha1Hash(0, pieceOffset).ToHexaDecimalString());
    ...
    private static PieceStatus GetStatus(bool ignore, bool download, string pieceHash, string calculatedPieceHash)
    {
    ...
    private static void Read(FileStream stream, long offset, int length, byte[] buffer, int bufferOffset)
    {
        ...
        if (stream.Length >= offset + length)
        {
            ...
            stream.ReadExactly(buffer, bufferOffset, length);
        }
        else
        {
