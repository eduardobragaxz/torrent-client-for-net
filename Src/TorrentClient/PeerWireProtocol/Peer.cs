...
using TorrentClient.PeerWireProtocol.EventArgs;
...
public sealed class Peer : IDisposable
{
    ...
    /// </summary>
    private readonly Lock downloadMessageQueueLocker = new();
    ...
    /// </summary>
    private readonly Lock sendMessageQueueLocker = new();
    ...
    /// </summary>
    private readonly Lock uploadMessageQueueLocker = new();
    ...
    public Peer(PeerCommunicator communicator, PieceManager pieceManager, string localPeerId, string peerId = null)
    {
        ...
        // send handshake
        EnqueueSendMessage(new HandshakeMessage(this.pieceManager.TorrentInfoHash, localPeerId, HandshakeMessage.ProtocolName));
    }
    ...
    private void Download()
    {
        ...
        if (!isDownloading)
        {
            ...
            while (!IsDisposed)
            {
                ...
                // process messages
                foreach (PeerMessage message in DequeueDownloadMessages())
                {
    ...
    private void OnCommunicationErrorOccurred(object sender, PeerCommunicationErrorEventArgs e)
    {
        ...
        CommunicationErrorOccurred?.Invoke(sender, e);
    }
    ...
    private void UpdateTrafficParameters(long downloaded, long uploaded)
    {
        ...
        lock (locker)
        {
            ...
            {
                downloadSpeed = (decimal)previouslyDownloaded / (decimal)stopwatch.Elapsed.TotalSeconds;
                uploadSpeed = (decimal)previouslyUploaded / (decimal)stopwatch.Elapsed.TotalSeconds;
    ...
    private void Upload()
    {
        ...
        if (!isUploading)
        {
            ...
            while (!IsDisposed)
            {
                foreach (PeerMessage message in DequeueUploadMessages())
                {
