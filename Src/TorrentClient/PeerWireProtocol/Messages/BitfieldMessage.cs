...
public class BitFieldMessage : PeerMessage
{
    ...
    public BitFieldMessage(bool[] bitField)
    {
        ...
        payloadLength = (int)Math.Ceiling((decimal)bitField.Length / (decimal)8);
            ...
            offsetTo <= buffer.Length)
        {
                ...
                messageId == MessageId)
            {
                if (offsetFrom + messageLength - MessageIdLength <= offsetTo)
                {
                    ...
                    {
                        message = new BitFieldMessage([.. new BitArray(payload).Cast<bool>()]);
                    }
                }
                else
                {
    ...
    {
        byte[] byteField = new byte[payloadLength];
        int written = offset;
    ...
    {
        if (obj is not BitFieldMessage bf)
        {
            return false;
        }
        else
