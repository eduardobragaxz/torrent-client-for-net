...
public sealed class TorrentClient : IDisposable
{
    ...
    /// </summary>
    private readonly Dictionary<string, TransferManager> transfers = [];
    ...
    public TorrentProgressInfo? GetProgressInfo(string torrentInfoHash)
    {
        ...
        if (transfers.TryGetValue(torrentInfoHash, out TransferManager? transferManager))
        {
    ...
    public void Start()
    {
        ...
        // start listening
        server = new TcpListener(IPAddress.Any, (int)ListeningPort);
        server.Start();
    ...
    public void Stop(string torrentInfoHash)
    {
        ...
        lock (((IDictionary)transfers).SyncRoot)
        {
            if (transfers.TryGetValue(torrentInfoHash, out TransferManager? transfer))
            {
                ...
                if (bytesRead > 0)
                {
                    foreach (var message in PeerMessage.Decode(buffer, ref offset, bytesRead))
                    {
                        if (message is HandshakeMessage)
                        {
                            lock (((IDictionary)transfers).SyncRoot)
                            {
                                if (transfers.TryGetValue(message.As<HandshakeMessage>().InfoHash, out TransferManager? transfer))
                                {
